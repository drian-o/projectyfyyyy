            ...
            <div class="col-md-12">
                <div class="d-flex no-block align-items-center">
                    ...
                    <div class="ml-auto">
                        ...
                        // Fetch data from API
                        $response = $ZH->InfoAgent();
                        $data = json_decode($response, true);

                        if ($data['status'] == 'success') {
                            $balance = $data['data']['balance'];
                            $formatted_balance = number_format($balance);
                            echo "<h1 class='font-light text-right' style='color: #137eff!important;'>{$formatted_balance}</h1>";
                        } else {
                            // Handle error case
                            echo "<h1 class='font-light text-right'>Error: " . $data['msg'] . "</h1>";
                        }
                        ?>
                    </div>
                </div>
